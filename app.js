...
function pathFinder(maze) {
  var start = [
    document.getElementById('start-cell').parentNode.rowIndex,
    document.getElementById('start-cell').cellIndex,
  ];
  var end = [
    document.getElementById('end-cell').parentNode.rowIndex,
    document.getElementById('end-cell').cellIndex,
  ];
  ...
  if (maze[end[0]][end[1]] == '.') {
    ...
    instructions.innerHTML =
      'this maze was solved in ' + maze[end[0]][end[1]] + ' moves';
...
function paintpath(maze, length, i, j) {
  document
    .getElementById('init-table')
    .rows[i].cells[j].classList.add('maze-path');
  var nextValue = maze[i][j] - 1;
  if (i + 1 < length) {
    if (maze[i + 1][j] == nextValue) {
      document
        .getElementById('init-table')
        .rows[i + 1].cells[j].classList.add('maze-path');
  ...
  if (j + 1 < length) {
    if (maze[i][j + 1] == nextValue) {
      document
        .getElementById('init-table')
        .rows[i].cells[j + 1].classList.add('maze-path');
...
function cellClick() {
  ...
  if (step === 'start') {
    setStart(this);
    instructions.innerHTML = 'set end point';
    step = 'end';
  }
}
function setStart(cell) {
  let existingCell = document.getElementById('start-cell');
  if (existingCell !== null) {
    existingCell.id = '';
    existingCell.innerText = '';
  }
  cell.innerText = 'start';
  cell.id = 'start-cell';
}
function setEnd(cell) {
  let existingCell = document.getElementById('end-cell');
  if (existingCell !== null) {
    existingCell.id = '';
    existingCell.innerText = '';
  }
...
function resetResults() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = 0; j < cells.length; j++) {
      if (cells[j].classList.contains('maze-border') == false) {
        cells[j].removeAttribute('class');
        if (
          cells[j].classList.contains('maze-border') == false &&
          cells[j].id != 'start-cell' &&
          cells[j].id != 'end-cell'
        ) {
          cells[j].innerText = '';
        }
...
function resetTable() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = 0; j < cells.length; j++) {
      ...
      cells[j].innerText = '';
      step = 'start';
      instructions.innerHTML = 'set start point';
    }
  }
}
window.onload = (event) => {
...
function returnToSetStart() {
  resetResults();
  step = 'resetStart';
  instructions.innerHTML = 'set start point';
}

function returnToSetEnd() {
  resetResults();
  step = 'resetEnd';
  instructions.innerHTML = 'set end point';
}
